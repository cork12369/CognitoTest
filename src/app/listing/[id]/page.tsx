        ...
        <main>
            ...
            <section className="detail-content">
                <div className="detail-main"><section className="content-section"><h2>About this listing</h2><p>{listing.description}</p><div className="tag-row">{listing.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></section>
                    <section className="compatibility-panel"><div className="panel-icon"><SparkleIcon /></div><div><p className="eyebrow">Loop Check</p><h2>{listing.compatibility}</h2><p>{listing.compatibilitySummary}</p></div></section>
                    <section className="signal-section"><div><p className="eyebrow"><span className="eyebrow-dot" /> Loop Check map</p><h2>What it connects to.</h2><p>Structured compatibility notes are illustrative and limited to the seeded product record.</p></div><div className="signal-map"><div className="signal-node product-node"><span>{listing.brand.slice(0, 1)}</span><strong>{listing.model}</strong><small>{listing.ports[0]}</small></div><div className="signal-line" /><div className="signal-node target-node"><span>↔</span><strong>Loop Check match</strong><small>{listing.compatibilitySummary}</small></div></div></section>
            ...
            {related.length > 0 && <section className="related-section"><div><p className="eyebrow"><span className="eyebrow-dot" /> Smart Swap</p><h2>Compare similar gear.</h2></div><div className="related-links">{related.map((item) => <Link href={`/listing/${item.id}`} key={item.id}><span>{item.brand}</span><strong>{item.title}</strong><em>{money(item.price)}</em><ArrowIcon /></Link>)}</div></section>}
